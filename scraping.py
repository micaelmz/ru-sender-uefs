...
if today_date.weekday() == 0 or not menu_updated_yet():
    ...
    df = pd.read_csv("last_update.csv")
    df['menu_name'] = link.split('/')[-1].replace('.pdf', '')
    ...
    # upadte last update
    df = pd.read_csv("last_update.csv")
    df["last_update"] = today_date.strftime("%d/%m/%Y %H:%M:%S")
    df.to_csv("last_update.csv", index=False)
    print("last update updated!")

else:
    print("O cardápio já foi atualizao essa semana")